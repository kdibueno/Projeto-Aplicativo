...
const plans = {
  desenvolvedor: [
    ...
    },
    {
      ...
      tasks: [
        "Desenvolver app de tarefas",
        "Revisar variáveis, funções e DOM"
      ]
    },
    {
      ...
      tasks: [
        "Finalizar mini-projetos",
        "Revisar eventos e funções assíncronas"
      ]
    },
    {
    ...
    },
    {
      ...
      tasks: [
        "Aprender fundamentos TS",
        "Aplicar TS em componentes React"
      ]
    },
    {
      ...
      tasks: [
        "Criar painel admin ou lista de tarefas com filtros"
      ]
    },
    {
    ...
    },
    {
      ...
      tasks: [
        "Estudar MongoDB",
        "Desenvolver API de blog com usuários"
      ]
    },
    {
      ...
      tasks: [
        "Unir React frontend com backend Node"
      ]
    },
    {
      ...
      tasks: [
        ...
        "Deploy backend no Render ou Railway"
      ]
    },
    {
 week: "Semana 20 (Dez 9 – Dez 15)",
      ...
      tasks: [
        ...
        "Organizar portfólio para apresentação"
      ]
    }
    // ... outras semanas do desenvolvedor aqui
  ],
...
fitness: [
    ...
    // ... outras semanas do fitness aqui
  ]
};
...
function PomodoroTimer() {
    ...
    const timer = setInterval(() => {
      setSecondsLeft((prev) => {
        if (prev <= 1) {
          clearInterval(timer);
          playSound();
          sendReminderNotification(isBreak ? "Pausa finalizada! Volte ao foco." : "Pomodoro finalizado! Faça uma pausa.");
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="flex flex-col items-center gap-4 p-4 border rounded-xl dark:border-zinc-700 shadow-xl bg-white dark:bg-zinc-800">
      <h3 className="text-lg font-semibold">Pomodoro</h3>
      <div className="flex gap-2">
        <label className="text-sm">Foco:</label>
        <Input type="number" min="1" value={focusMinutes} onChange={(e) => setFocusMinutes(Number(e.target.value))} className="w-16" />
        <label className="text-sm">Pausa:</label>
        <Input type="number" min="1" value={breakMinutes} onChange={(e) => setBreakMinutes(Number(e.target.value))} className="w-16" />
      </div>
      <div className="text-3xl font-mono">{minutes}:{seconds}</div>
      <div className="flex gap-2">
        <Button onClick={() => setIsRunning(true)}><Timer className="w-4 h-4 mr-1" /> Iniciar</Button>
        <Button variant="secondary" onClick={() => setIsRunning(false)}>Pausar</Button>
        <Button variant="outline" onClick={() => { setIsRunning(false); setSecondsLeft(focusMinutes * 60); setIsBreak(false); }}>Resetar</Button>
      </div>
...
export default function FullstackPlanApp() {
  const [completed, setCompleted] = useLocalStorage("completed", { desenvolvedor: Array(plans.desenvolvedor.length).fill(0), fitness: Array(plans.fitness.length).fill(0) });
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto space-y-6 transition-colors duration-300 bg-zinc-100 dark:bg-zinc-900">
